...
class TerminalTest(unittest.TestCase):
  ...
  def testAnsiCmd(self):
    self.failUnlessEqual('\033[0m', terminal._AnsiCmd(['reset']))
    self.failUnlessEqual('\033[0m', terminal._AnsiCmd(['RESET']))
    self.failUnlessEqual('\033[0;32m', terminal._AnsiCmd(['reset', 'Green']))
    self.failUnlessRaises(ValueError, terminal._AnsiCmd, ['bogus'])
    self.failUnlessRaises(ValueError, terminal._AnsiCmd, ['reset', 'bogus'])

  def testAnsiText(self):
    self.failUnlessEqual('\033[0mhello world\033[0m',
                         terminal.AnsiText('hello world'))
    self.failUnlessEqual('\033[31mhello world\033[0m',
                         terminal.AnsiText('hello world', ['red']))
    self.failUnlessEqual('\033[31;46mhello world',
                         terminal.AnsiText('hello world', ['red', 'bg_cyan'],
                                           False))

  def testStripAnsi(self):
    text = 'ansi length'
    self.failUnlessEqual(text, terminal.StripAnsiText(text))
  ...
  def testTerminalSize(self):
    ...
    self.failUnlessEqual((10, 20), terminal.TerminalSize())

  def testLineWrap(self):
    terminal.TerminalSize = lambda: (5, 11)
    text = ''
    self.failUnlessEqual(text, terminal.LineWrap(text))
    text = 'one line'
    self.failUnlessEqual(text, terminal.LineWrap(text))
    text = 'two\nlines'
    self.failUnlessEqual(text, terminal.LineWrap(text))
    ...
    text2 = 'one line th\nat is too l\nong'
    self.failUnlessEqual(text2, terminal.LineWrap(text))
    # Counting ansi characters won't matter if there are none.
    self.failUnlessEqual(text2, terminal.LineWrap(text, False))
    ...
    self.failUnlessEqual(text2, terminal.LineWrap(text, True))
    # If we count the ansi escape as characters then the line breaks change.
    self.failUnlessEqual(text3, terminal.LineWrap(text, False))
    # False is implicit default.
    self.failUnlessEqual(text3, terminal.LineWrap(text))
    ...
    self.failUnlessEqual(text5, terminal.LineWrap(text4, True))
